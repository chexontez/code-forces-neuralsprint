...
# Убираем прямой импорт DatabaseManager, он больше не нужен клиенту для входа
...
class LoginWindow(QMainWindow):
    ...
    # Убираем db_manager из конструктора
    def __init__(self):
        super().__init__()
    ...
    def handle_login(self):
        ...
        # --- Новая логика: отправка запроса на сервер ---
        try:
            response = requests.post(f"{self.server_url}/login", 
                                     json={"username": username, "password": password},
                                     timeout=5) # таймаут 5 секунд
            
            # Проверяем, что сервер ответил успешно (код 2xx)
            if response.status_code == 200:
                ...
                if data.get("success"):
                    ...
                    # Эта ветка больше не нужна, т.к. сервер вернет 401
                    pass
